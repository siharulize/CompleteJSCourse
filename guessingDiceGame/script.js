...
questionsList.push(new Question("What is the name of my Cat?", ["Lilu", "Max", "Meercat"], 0));
questionsList.push(new Question("What is my favourite Fruit?", ["Apple", "Blueberry", "Melon", "Peach"], 1));
questionsList.push(new Question("What kind of weather do I like?", ["Rainy", "Snowy", "Sunny"], 2));


function game(arr){
	var num = Math.floor(Math.random()* arr.length);
	arr[num].display();
	var guess = prompt("Enter Correct Answer");

	if (guess === "exit") {
		console.log("game finished");
		return;
	} else {
		arr[num].checkAnswer(parseInt(guess));
